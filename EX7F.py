...
# C
def SearchC(x: int, head: int, val: list[int], P: list[int]):
    # 这里只是将B算法中的选择方式从改成了随机选择
    i = head
    max = val[i]
    global count
    for j in range(1, int(np.sqrt(len(val))) + 1):
...
if __name__ == '__main__':
    ...
    print(cnt_lst_A)
    # 输出平均运行次数
    print(np.mean(cnt_lst_A))
    # 输出最小运行次数
    print(np.min(cnt_lst_A))
    # 输出最大运行次数
    print(np.max(cnt_lst_A))
    ...
    for i in range(0, 1000):
        ...
        # print(val)
        P, head = findP(val)
        count_lst_B.append(SearchB(random.randint(0, 9999), head, val, P)[1])
        print(count_lst_B)
    ...
    for i in range(0, 1000):
        ...
        count_lst_C.append(count)
        print("第%d轮" % i, count_lst_C)
    cnt_lst_C = np.array(count_lst_C)
    print(cnt_lst_C)
    print(np.average(cnt_lst_C))
    print(np.min(cnt_lst_C))
    print(np.max(cnt_lst_C))
